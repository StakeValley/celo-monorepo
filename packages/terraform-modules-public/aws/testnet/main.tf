...
locals {
  ...
  validator_params = {
    az1 = zipmap(
      ...
      [for k, v in var.validators.az1 : merge(var.validators.az1[k], lookup(local.proxy_settings.az1, k, {}))]
    )
    az2 = zipmap(
      keys(var.validators.az2),
      [for k, v in var.validators.az2 : merge(var.validators.az2[k], lookup(local.proxy_settings.az2, k, {}))]
    )
  }
...
locals {
  attestation_db_url = length(aws_db_instance.attestation) > 0 ? format("postgresql://%s:%s@%s/%s",
    ...
    aws_db_instance.attestation[0].db_name
  ) : ""

  attestation_service_params = {
    az1 = zipmap(
      keys(var.attestation_services.az1),
      [for k, v in var.attestation_services.az1 : merge(var.attestation_services.az1[k], lookup(local.proxy_settings.az1, k, {}))]
    )
    az2 = zipmap(
      keys(var.attestation_services.az2),
      [for k, v in var.attestation_services.az2 : merge(var.attestation_services.az2[k], lookup(local.proxy_settings.az2, k, {}))]
    )
  }
}
...
module "celo_attestation_service_az1" {
  ...
  nexmo_api_key                                 = "TODO" #var.nexmo_api_key
  nexmo_api_secret                              = "TODO" #var.nexmo_api_secret
  nexmo_unsupported_regions                     = "TODO" #var.nexmo_unsupported_regions
  messagebird_api_key                           = "TODO"
  messagebird_unsupported_regions               = "TODO"
